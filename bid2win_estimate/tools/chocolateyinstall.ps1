...
$b2wServer = 'RYBAPRIMARY\BID2WIN'
$b2wDatabase = 'B2WData'

# install options which apply to all types
$genericSilentArgs = @(
  '/IACCEPTB2WSETUPLICENSETERMS=true'
  '/WizardAction=FullInstall'
  "/InstallSixtyFourBitClient=$is64bit"
  '/ProgramInstallDirectory=C:\Program Files\B2W Software\Estimate\BID2WIN.exe'
  '/ShortcutsForAllUsers=true'
)

switch -Regex ($pp.type)
{
    '^server$' {
      $additionalSilentArgs = @(
        '/InstallNetworkServer_SingleUser=true'
        '/LocalB2WServer=(local)\BID2WIN'
      )
    }
    '^client$' {
      $additionalSilentArgs = @(
        '/InstallNetworkClient=true'
        "/DefaultB2WServer=$b2wServer"
        "/DefaultB2WDatabase=$b2wDatabase"
      )
    }
    '^mobile$' {
      $additionalSilentArgs = @(
        '/InstallMobileClient=true'
        "/DefaultB2WServer=$b2wServer"
        "/DefaultB2WDatabase=$b2wDatabase"
        '/InstallSQLExpress2012=true'
      )  
    }
    default { Break }
}
...
$allSilentArgs = @()
$allSilentArgs += , $genericSilentArgs
$allSilentArgs += , $additionalSilentArgs

Write-Host "Installation type: $($pp.type)"

Write-Host "Resulting args: $allSilentArgs"
...
$fileLocation = Join-Path $toolsDir 'B2WEstimateSetupWizard21.1.7823.18409.exe'
$configfileLocation = Join-Path $toolsDir 'B2WSetupConfig.ini'
...
$packageArgs = @{
  ...
  softwareName  = 'B2W Estimate' # part or all of the Display Name as you see it in Programs and Features. It should be enough to be unique

  # Checksums are now required as of 0.10.0.
  # To determine checksums, you can get that from the original site if provided.
  # You can also use checksum.exe (choco install checksum) and use it
  # e.g. checksum -t sha256 -f path\to\file
  checksum      = '0476E7303ED70382C46B49C5460B3A6095EDB3BC3862555AF51905CBCE84107E'
  ...
  # silentArgs   = "/config=$configfileLocation"  # none; make silent with input macro script like AutoHotKey (AHK)
  #       https://community.chocolatey.org/packages/autohotkey.portable
  silentArgs = $allSilentArgs
